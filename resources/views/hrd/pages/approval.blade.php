...
@section('page_subtitle', 'Tinjau dan proses persetujuan pengajuan FPTK dari HOD.')
...
@section('content')
    ...
    @php
        ...
        $pendingFptk = ($summary['approved_gm'] ?? 0) + ($summary['revisi_hrd'] ?? 0);
    @endphp
    ...
    <div class="space-y-4">
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
            ...
            <div class="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center gap-3">
                <div class="flex-1">
                    ...
                    <div class="text-xs text-gray-400 mt-0.5">
                        FPTK yang sudah disetujui GM dan siap ditinjau HRD.
                    </div>
                </div>

                @if ($pendingFptk > 0)
                    <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-red-50 border border-red-200 text-red-600 text-xs font-semibold shrink-0">
                        <i class="bi bi-exclamation-circle-fill text-sm"></i>
                        {{ $pendingFptk }} FPTK menunggu tindakan
                    </div>
                @endif
            </div>
            ...
            <div class="px-5 py-3 border-b border-gray-100 bg-gray-50/40">
                <form method="GET" action="{{ route('hrd.approval.index') }}" id="filterForm"
                    ...
                    <select name="status" onchange="document.getElementById('filterForm').submit()"
                        ...
                        @foreach ($statusFptkMap as $val => [$cls, $icon, $lbl])
                            <option value="{{ $val }}" {{ request('status') === $val ? 'selected' : '' }}>{{ $lbl }}</option>
                        @endforeach
                    </select>

                    <button type="submit"
                        class="px-4 py-2.5 rounded-xl bg-[#145D9E] text-white text-sm font-semibold hover:bg-[#0f4d85] transition inline-flex items-center gap-2 shrink-0 cursor-pointer">
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 text-gray-500 border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-semibold">Posisi Dibutuhkan</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($fptks as $i => $item)
                            @php
                                [$cls, $icon, $lbl] = $statusFptkMap[$item->status]
                                    ?? ['bg-gray-100 border-gray-200 text-gray-500', 'bi-dash-circle', $item->status];
                            @endphp
                            <tr class="hover:bg-gray-50/70 transition">
                                <td class="px-4 py-3 text-gray-400 text-xs font-medium text-center">
                                    {{ $fptks->firstItem() + $i }}
                                </td>
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center rounded-full border border-indigo-200 bg-indigo-50 text-indigo-700 px-3 py-1 text-xs font-semibold font-mono">
                                        {{ $item->nomor_fptk }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        <div class="w-7 h-7 rounded-full bg-[#145D9E]/10 text-[#145D9E] text-xs font-bold flex items-center justify-center shrink-0">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center rounded-full border border-sky-200 bg-sky-50 text-sky-700 px-3 py-1 text-xs font-semibold">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold {{ $cls }}">
                                        <i class="bi {{ $icon }} text-[9px]"></i> {{ $lbl }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <button type="button"
                                            onclick='bukaDetailFptk(@json($item->load("departemen", "hod")))'
                                            title="Detail"
                                        ...
                                        @if (in_array($item->status, ['approved_gm']))
                                            <button type="button"
                                                onclick="konfirmasiApproveFptk({{ $item->id }}, '{{ $item->nomor_fptk }}')"
                                                title="Setujui"
                                            ...
                                            <button type="button"
                                                onclick='bukaModalBatalkan(@json($item->only(["id", "nomor_fptk", "posisi_dibutuhkan"])))'
                                                title="Tolak"
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            <div class="text-sm font-semibold text-gray-600">Tidak ada FPTK yang perlu ditinjau</div>
                                            <div class="text-xs text-gray-400 mt-0.5">Semua FPTK sudah diproses atau belum ada yang masuk.</div>
                                        </div>
                                        @if (request('search') || request('status'))
                                            <a href="{{ route('hrd.approval.index') }}"
            ...
            <div class="px-5 py-3.5 border-t border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="flex items-center gap-2 text-xs text-gray-500">
                    ...
                    @if ($fptks->count() > 0)
                        <span>menampilkan
                            <span class="font-semibold text-gray-700">{{ $fptks->firstItem() }}</span> &ndash;
                            <span class="font-semibold text-gray-700">{{ $fptks->lastItem() }}</span> dari
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        @else
                            <a href="{{ $paginated->previousPageUrl() }}"
                                class="w-8 h-8 rounded-lg border border-gray-200 text-gray-500 bg-white flex items-center justify-center hover:bg-blue-50 hover:border-blue-200 hover:text-[#145D9E] transition">
                                <i class="bi bi-chevron-left text-xs"></i>
                            </a>
                        @endif
                        ...
                        @foreach ($range as $idx => $page)
                            ...
                            @endif
                            @if ($page === $currentPage)
                                <span class="w-8 h-8 rounded-lg text-xs font-semibold bg-[#145D9E] text-white flex items-center justify-center shadow-sm">{{ $page }}</span>
                            @else
                                <a href="{{ $paginated->url($page) }}"
                                    class="w-8 h-8 rounded-lg text-xs font-semibold border border-gray-200 bg-white text-gray-600 flex items-center justify-center hover:bg-blue-50 hover:border-blue-200 hover:text-[#145D9E] transition">{{ $page }}</a>
                            @endif
                        @endforeach
        ...
        <div class="relative bg-white rounded-2xl w-full max-w-lg mx-4 shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div class="overflow-y-auto px-6 py-5 space-y-5">
                <div>
                    ...
                    <div id="fd_nomor" class="inline-flex items-center rounded-full border border-indigo-200 bg-indigo-50 text-indigo-700 px-3 py-1 text-xs font-semibold font-mono"></div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div id="fd_departemen" class="inline-flex items-center rounded-full border border-blue-200 bg-blue-50 text-blue-700 px-3 py-1 text-xs font-semibold"></div>
                    </div>
                    <div>
    ...
    {{-- MODAL — Tolak FPTK  --}}
    <div id="modalBatalkanFptk" class="fixed inset-0 z-[60] flex items-center justify-center hidden">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" onclick="closeModal('modalBatalkanFptk')"></div>
        <div class="relative bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl flex flex-col">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
                <div class="flex items-center gap-3">
                ...
                <button type="button" onclick="closeModal('modalBatalkanFptk')"
                    class="w-8 h-8 rounded-xl border border-gray-200 bg-red-600 text-white flex items-center justify-center hover:bg-red-700 cursor-pointer transition">
                    <i class="bi bi-x-lg text-xs"></i>
                </button>
            </div>
            <div class="px-6 py-5 space-y-4">
                <div class="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-50 border border-red-100">
                    ...
                    <div>
                        <div id="fb_nomor" class="text-xs font-semibold text-red-700 font-mono"></div>
                        <div id="fb_posisi" class="text-sm font-semibold text-gray-800 mt-0.5"></div>
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1.5">
                    ...
                    <textarea id="fb_catatan" rows="4" placeholder="Jelaskan alasan pembatalan FPTK ini..."
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition resize-none"></textarea>
                    <p class="mt-1.5 text-xs text-gray-400">
                        <i class="bi bi-info-circle"></i> Alasan ini akan tercatat di riwayat FPTK.
                    </p>
                </div>
            </div>
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-end gap-2 shrink-0">
                ...
                <button type="button" onclick="kirimBatalkan()"
    ...
    {{-- Hidden Forms --}}
    <form id="formApproveFptk" method="POST" action="" style="display:none">
        @csrf @method('PATCH')
    </form>
    <form id="formRevisiFptk" method="POST" action="" style="display:none">
        @csrf @method('PATCH')
        <input type="hidden" name="catatan_hrd" id="formRevisi_catatan">
    </form>
    <form id="formBatalkanFptk" method="POST" action="" style="display:none">
        @csrf @method('PATCH')
        <input type="hidden" name="catatan_hrd" id="formBatalkan_catatan">
    </form>

    @push('scripts')
        <style>
            #fd_timeline li:last-child .timeline-line { display: none; }
        </style>
        ...
        <script>
            ...
            function closeModal(id) {
                ...
                var inputs = { modalRevisiFptk: 'fr_catatan', modalBatalkanFptk: 'fb_catatan' };
                if (inputs[id]) document.getElementById(inputs[id]).value = '';
            }
            ...
            function badgeStatus(key) {
                var s = statusFptkMap[key] || { cls: 'bg-gray-100 border-gray-200 text-gray-500', icon: 'bi-dash-circle', label: key };
                return '<span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ' + s.cls + '">'
            ...
            function buatLangkahTimeline(state, judul, tanggal, catatan, alertStyle) {
                ...
                var banner = catatan
                    ? '<div class="mt-2 flex items-start gap-2 px-3 py-2.5 rounded-lg '
                        + (alertStyle ? 'bg-orange-50 border border-orange-200' : 'bg-gray-50 border border-gray-100') + '">'
                ...
                return '<li class="relative flex gap-4 pb-5 last:pb-0">'
                    ...
                    + '<div class="w-px flex-1 mt-1 timeline-line ' + cfg.line + '"></div></div>'
            ...
            function bukaDetailFptk(item) {
                ...
                document.getElementById('fd_departemen').textContent = item.departemen
                ...
                document.getElementById('fd_status').innerHTML       = badgeStatus(item.status);
                ...
                var bisaAksi = ['approved_gm', 'revisi_hrd'].includes(item.status);
                document.getElementById('fd_aksi').innerHTML = bisaAksi
                    ? '<button type="button" onclick=\'closeModal("modalDetailFptk");konfirmasiApproveFptk(' + item.id + ',"' + item.nomor_fptk + '")\''
            ...
            /* ── Approve ── */
            function konfirmasiApproveFptk(id, nomorFptk) {
                Swal.fire({
                    title: 'Setujui FPTK?',
                    html: '<div class="text-left text-sm"><div><strong>Nomor:</strong> ' + nomorFptk + '</div>'
                        + '<div class="text-gray-500 text-xs mt-1">Proses rekrutmen dapat dimulai setelah disetujui.</div></div>',
                    ...
                    if (r.isConfirmed) {
                        var f = document.getElementById('formApproveFptk');
                        f.action = '{{ url('/hrd/approval') }}/' + id + '/approve';
                        f.submit();
                    }
                });
            }
            ...
            function bukaModalRevisi(item) {
                ...
                document.getElementById('formRevisiFptk').action = '{{ url('/hrd/approval') }}/' + item.id + '/revisi';
            ...
            function kirimRevisi() {
                ...
                if (!catatan) {
                    Swal.fire({ icon: 'warning', title: 'Catatan wajib diisi', text: 'Jelaskan kepada HOD apa yang perlu diperbaiki.', confirmButtonColor: '#145D9E' });
                    return;
                }
                Swal.fire({
                    title: 'Kirim Catatan Revisi?', text: 'HOD akan diminta memperbaiki dan mengajukan ulang.',
                    icon: 'question', showCancelButton: true,
                    confirmButtonColor: '#f97316', cancelButtonColor: '#6B7280',
                    confirmButtonText: 'Ya, Kirim!', cancelButtonText: 'Cek Lagi', reverseButtons: true,
                }).then(function (r) {
                    if (r.isConfirmed) {
                        document.getElementById('formRevisi_catatan').value = catatan;
                        document.getElementById('formRevisiFptk').submit();
                    }
                });
            }

            /* ── Tolak ── */
            function bukaModalBatalkan(item) {
                document.getElementById('fb_nomor').textContent = item.nomor_fptk;
                document.getElementById('fb_posisi').textContent = item.posisi_dibutuhkan;
                document.getElementById('fb_catatan').value = '';
                document.getElementById('formBatalkanFptk').action = '{{ url('/hrd/approval') }}/' + item.id + '/batalkan';
                openModal('modalBatalkanFptk');
            }

            function kirimBatalkan() {
                var catatan = document.getElementById('fb_catatan').value.trim();
                if (!catatan) {
                    Swal.fire({ icon: 'warning', title: 'Alasan wajib diisi', text: 'Berikan alasan pembatalan FPTK ini.', confirmButtonColor: '#145D9E' });
                    return;
                }
                Swal.fire({
                    title: 'Tolak FPTK?', text: 'Tindakan ini tidak dapat diurungkan.',
                    icon: 'warning', showCancelButton: true,
                    confirmButtonColor: '#DC2626', cancelButtonColor: '#6B7280',
                    confirmButtonText: 'Ya, Tolak!', cancelButtonText: 'Kembali', reverseButtons: true,
                }).then(function (r) {
                    if (r.isConfirmed) {
                        document.getElementById('formBatalkan_catatan').value = catatan;
                        document.getElementById('formBatalkanFptk').submit();
                    }
                });
            }
